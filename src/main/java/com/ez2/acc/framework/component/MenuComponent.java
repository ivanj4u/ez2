...
import com.ez2.acc.view.user.DetailUserPasswordView;
import com.ez2.acc.view.user.DetailUserProfileView;
...
//            menuItemsLayout.addStyleName(ValoTheme.MENU_ITEM);
    ...
    private EzUser getCurrentUser() {
        return VaadinSession.getCurrent()
                .getAttribute(EzUser.class);
    }
    ...
    private void buildMenuItem() {
        if (menuItemsLayout.getComponentCount() == 0) {
            try {
                hMenu.clear();
                for (final EzMenu view : menuLoader.getAuthorizedMenu()) {
    ...
    private void doLogout() {
        ...
        VaadinSession.getCurrent().setAttribute(EzUser.class.getName(), null);
